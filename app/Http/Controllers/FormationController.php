...
class FormationController extends Controller
{
    ...
    /** Télécharger le catalogue PDF des formations (filtré par type optionnellement) */
    public function downloadCatalogue(Request $request)
    {
        $type = $request->query('type');

        $query = Formation::with(['pays'])
            ->orderBy('categorie')
            ->orderBy('date_debut');

        if ($type && in_array($type, ['etudiant', 'particulier', 'entreprise'])) {
            $query->where('categorie', $type);
        }

        $formations = $query->get()->groupBy('categorie');
        ...
        $filename = $type
            ? "catalogue-formations-{$type}-isd-afrik.pdf"
            : 'catalogue-formations-isd-afrik.pdf';
